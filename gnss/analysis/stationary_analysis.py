...
def analyze_stationary_data(open_df, occluded_df):
    ...
    open_easting = open_easting - open_easting[0]
    ...
    # Annotate centroid coordinates in the plot
    mpl.text(open_centroid['easting'], open_centroid['northing'], 
             f"({open_centroid['easting']:.2f}, {open_centroid['northing']:.2f})", 
             fontsize=12, color='green', fontweight='bold', ha='right')
    
    mpl.text(occluded_centroid['easting'], occluded_centroid['northing'], 
             f"({occluded_centroid['easting']:.2f}, {occluded_centroid['northing']:.2f})", 
             fontsize=12, color='orange', fontweight='bold', ha='right')
    
    # Calculate standard deviation for Open data
    open_std_easting = nu.std(open_easting)
    open_std_northing = nu.std(open_northing)

    # Calculate standard deviation for Occluded data
    occluded_std_easting = nu.std(occluded_easting)
    occluded_std_northing = nu.std(occluded_northing)

    mpl.title('Stationary Northing vs. Easting Scatterplots')
    ...
    mpl.figure()
    mpl.plot(open_df['timestamp'].values, open_altitude, label='Open Spot', marker='o', color='blue')
    mpl.plot(occluded_df['timestamp'].values, occluded_altitude, label='Occluded Spot', marker='x', color='red')
...
if __name__ == '__main__':
    open_spot_df = load_rosbag('//home/chandra/catkin_ws/src/gnss/data/stationary_open.bag', '/gps')
    occluded_spot_df = load_rosbag('/home/chandra/catkin_ws/src/gnss/data/stationary_close.bag', '/gps')
